...
Original file is located at
    https://colab.research.google.com/drive/1sWU5faASMSHvwYsdA3DK1jxZ4cHw4ttz
"""
...
## Answer:
Batch normalization을 사용한 경우가 그렇지 않은 네트워크보다 global하게 성능이 좋았다. Batchnorm은 initializaition scale에 영향을 받는다는 것을 알 수 있었다. 이는 모든 initialization scale weights에 네트워크가 같은 epoch으로 학습이 되었는데 batchnorm layer는 더 높은 initialization scale을 가질때 scale을 학습하고 파라미터를 바꾸는데 더 오랜 시간이 걸리기 때문일 것이다. 그리고 3번째 그래프를 보면 batch normalization이 높은 initialization scale에서 explosion을 방지할 수 있다는 것을 알 수 있다.
...
## Answer:
Batch normalization은 training에 사용된 batch의 크기에 매우 의존적이다. Batch의 크기가 클수록 좋은 결과가 나온다.
...
## Answer:
2번. feature의 수가 적으면 부정확한 normalizing이 될 것이다. 
